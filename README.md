...
- **Роли**: Владелец (Соҳиб) — полный доступ; Продавец (Фурӯшанда) — оформляет продажи.
- **Товары (Молҳо)** — как каталог интернет-магазина: **фото**, категория, себестоимость, **цена продажи**, количество (остаток), единица измерения, описание, дата поступления; поиск, категории; общая стоимость. Фото сжимается на клиенте и хранится в БД.
...
- **Продажи (Фурӯшҳо)** — прямо со склада:
  - *Розничная (Чакана)*: количество + цена за единицу (подставляется из товара) → система считает сумму;
  - *Оптовая (Яклухт)*: количество + общая сумма → система считает цену за единицу;
  - оплата: наличные / банковская карта;
  - **автосписание** остатка товара, защита от продажи сверх остатка даже при параллельных запросах (пессимистичные блокировки).
- **Уведомления**: после каждой продажи — уведомление владельцу в системе **и в Telegram** (дата, время, продавец, товар, количество, сумма, оплата, тип).
- **Аналитика (Таҳлил)**: день / 7 дней / месяц / произвольный период; количество и сумма продаж, наличные vs карта, продано товаров, прибыль, график по дням, топ товаров, разрез по продавцам.
- **Экспорт в Excel (.xlsx)**: продажи (весь список или за период, с учётом фильтров) и товары.
- **Пользователи**: создание продавцов/владельцев, деактивация, смена пароля.
...
```
backend/src/
  auth/ users/ products/ sales/ analytics/
  notifications/ settings/ telegram/ export/ seed/
  common/ (guards, decorators, sale-message, numbers, tz)
  entities/ (User, Product, Sale, Notification, Setting, TelegramLinkCode)
frontend/src/
  app/ (providers, router, styles)
  pages/ (login, analytics, warehouse, sales, sale-new, users, notifications, settings)
  widgets/ (layout)
  features/ (sale-form, product-form, user-form, telegram)
  entities/ (user, product, sale, notification, analytics, settings, telegram)
